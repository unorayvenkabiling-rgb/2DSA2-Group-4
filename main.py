...
def select_material():
    ...
    """
    # Material properties matching instructions: Yield Strength (MPa), Young's Modulus (GPa)
    materials = {
        "1": ("Steel", 250.0, 200.0),
...
def run_calculator():
    ...
    cross_sectional_area = get_positive_float("Enter cross sectional area (A) in square meters [m^2]: ")
    ...
    # 3. Calculations (Fixed line 11 bug from original code: Stress = Force / Area)
    ...
    # 5. Output Display (Extending groupmate's layout)[cite: 1]
    print("\n" + "=" * 50)
    print(" Calculation results ")
    print("=" * 50)
...
def main():
    """Main loop for repeated calculations and graceful exit."""
    while True:
        try:
            run_calculator()
        ...
        # Repeated Calculations check[cite: 1]
